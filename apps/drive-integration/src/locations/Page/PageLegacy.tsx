...
export const PageLegacy = () => {
  const sdk = useSDK<PageAppSDK>();
  const modalOrchestratorRef = useRef<ModalOrchestratorLegacyHandle>(null);
  ...
  const { resumeWorkflow } = useWorkflowAgentLegacy({
    sdk,
    documentId: '',
    oauthToken: '',
  });
  ...
  const handleAiAccessRestored = () => {
    if (aiAccessDeniedMessage !== null) setAiAccessDeniedMessage(null);
  };
  ...
  const handleConnectGoogleDrive = async () => {
    ...
    try {
      ...
      if (isAiAccessDeniedError(error)) handleAiAccessDenied(error.message);
    }
  };
