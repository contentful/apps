...
- **Runtime:** Production Lambda can use a newer Node runtime; for **local** dev we use `nodejs16.x` so serverless-offline 8.x works. Do not change to nodejs18/nodejs22 in `serverless.yml` unless you upgrade serverless-offline (and possibly serverless) to versions that support them.
