...
export function createMockFieldSdk(
  ...
  const {
    ...
    liveInstallationParameters = installationParameters,
  } = options;
  ...
  return {
    ...
    cma: {
      appInstallation: {
        get: vi.fn(() =>
          Promise.resolve({
            parameters: liveInstallationParameters ?? installationParameters,
          })
        ),
        getForOrganization: vi.fn(() =>
          Promise.resolve({
            items: liveInstallationParameters
              ? [
                  {
                    sys: {
                      space: { sys: { id: 'test-space-id' } },
                      environment: { sys: { id: 'test-environment-id' } },
                    },
                    parameters: liveInstallationParameters,
                  },
                ]
              : [],
          })
