...
const RewriteGenerator = () => {
  const { setProviderData, localeNames, feature } =
  ...
  const handleGenerate = async (generateMessage: GenerateMessage) => {
    try {
      ...
      const userMessage = featureConfig[feature].prompt(
        inputText,
        rewritePromptData,
      );
  ...
  return (
    <Flex flexDirection="column">
      <Box ref={headerRef}>
        ...
        />
        <ButtonTextField
          inputValue={rewritePromptData}
          handleInputChange={setRewritePromptData}
        />
      </Box>
      <Output
