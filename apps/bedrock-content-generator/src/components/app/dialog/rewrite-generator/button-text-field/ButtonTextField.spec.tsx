...
describe("Button Text Field", () => {
  it("renders on the page", () => {
    ...
    expect(
      getByPlaceholderText('eg. "Shorter, casual, for Star Wars fans..."'),
    ).toBeTruthy();
    unmount();
  });
  ...
  it("should add or remove its label to the text field on click", async () => {
    ...
    for (let i = 0; i < randomButtonQueueWithDuplicates.length; i++) {
      ...
      await waitFor(() =>
        expect(mockHandleInputChange).toHaveBeenCalledTimes(i + 1),
      );

      if (oldTestInputValue.includes(lowerCaseButton)) {
        await waitFor(() =>
          expect(testInputValue).toEqual(
            oldTestInputValue.replace(`${lowerCaseButton}, `, ""),
          ),
        );
      } else {
        await waitFor(() =>
          expect(testInputValue).toEqual(
            `${oldTestInputValue}${lowerCaseButton}, `,
          ),
        );
      }
    }
