...
test('displays a player when the state has a signed playbackUrl and signed posterUrl', async () => {
  ...
  expect(wrapper.state('playbackUrl')).toMatch('https://stream.mux.com/test-playbackId123.m3u8?token=')
  expect(wrapper.state('posterUrl')).toMatch('https://image.mux.com/test-playbackId123/thumbnail.jpg?token=')
  expect(wrapper.find('Player')).toHaveLength(1)
});
...
test('displays an error when we have a signed playbackId but no signing keys', async () => {
  ...
  wrapper.instance().forceUpdate()
  await (wrapper.instance() as App).setSignedPlayback('test-playbackId123')
  expect(wrapper.state('error')).toEqual('Error: this asset was created with a signed playback ID, but signing keys do not exist for your account')
