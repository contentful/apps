/* eslint-disable @typescript-eslint/no-explicit-any */
import {
  createSignedPlaybackToken,
  createSignedThumbnailToken,
  createSignedStoryboardToken,
} from './signingTokens';

/*
 * This was a valid private key, but it has since been revoked
 */
const keyPrivate =
  '';

test('signedPlaybackId', () => {
  let id = createSignedPlaybackToken('playback-id', 'key-id', keyPrivate);
  expect(true).toBe(typeof id === 'string' && id.length > 0);
});
test('createSignedThumbnailToken', () => {
  let id = createSignedThumbnailToken('playback-id', 'key-id', keyPrivate);
  expect(true).toBe(typeof id === 'string' && id.length > 0);
});
test('createSignedStoryboardToken', () => {
  let id = createSignedStoryboardToken('playback-id', 'key-id', keyPrivate);
  expect(true).toBe(typeof id === 'string' && id.length > 0);
});
