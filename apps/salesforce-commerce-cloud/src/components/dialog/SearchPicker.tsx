...
const SearchPicker = () => {
  ...
  const { selectMultiple, fieldType, fieldValue, currentData } = sdk.parameters.invocation as DialogInvocationParameters
  ...
  useEffect(() => {
    const timeOutId = setTimeout(() => {
      ...
      const fetchResults = fieldType === 'product' ? client.searchProducts : client.searchCategories
      if (query.length >= 3) {
  ...
  const SearchResultsComponent = fieldType === 'product' ? ProductSearchResults : CategorySearchResults;
