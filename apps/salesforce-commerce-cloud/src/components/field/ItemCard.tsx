    ...
    >
      <Flex flexDirection="column">
        <Flex alignItems="center" padding="spacingXs">
          {itemData ? (
            <ItemPreview id={props.id} type={props.type} itemData={itemData} onRemove={props.onRemove} />
          ) : (
        ...
        <Text as="div" fontWeight="fontWeightMedium" fontSize='fontSizeS' fontColor='gray600'>
          ...
          {props.type === 'category' && (
            <>
              ...
              <Box as="span" css={categoryStyle}>Catalog: {itemData.name?.default ? itemData.name.default : itemData.catalogId}</Box>
            </>
          )}
        </Text>
...
const MissingItem = (props:ItemProps) => {
  return (
    <>
      ...
      <Text marginLeft="spacingS">{props.type.charAt(0).toUpperCase() + props.type.slice(1)} not found: {props.id}</Text>
