...
class SfccClient {
  ...
  private fetchAccessToken = async () => {
    ...
    const { data } = await axios.post('https://account.demandware.com/dwsso/oauth2/access_token',
      {
  ...
  public searchCategories = async (query?:string) => {
    ...
    const { data: searchResults } = await this.client.post(`/product/catalogs/v1/organizations/${organizationId}/category-search`, data)
  ...
  public fetchCategory = async (catalogId:string, categoryId:string) => {
    const {data: category} = await this.client.get(`/product/catalogs/v1/organizations/${this.parameters.organizationId}/catalogs/${catalogId}/categories/${categoryId}`)
