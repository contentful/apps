...
function assertServiceAccountKey(value: any): asserts value is ServiceAccountKey {
  ...
  const notStringKeys = reject(keysOfServiceAccountKey, (key) => typeof value[key] === 'string');
  if (notStringKeys.length > 0)
    throw new AssertionError(
      `Key file has invalid values at the following keys: ${notStringKeys.join(', ')}`
    );
}

const FormControlServiceAccountKeyFile = ({
...
export default FormControlServiceAccountKeyFile;
