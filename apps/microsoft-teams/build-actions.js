...
const main = async (watch = false) => {
  try {
    ...
    const config = {
      ...
      external: ['node:*'],
      define: {
        'process.env.VITE_MSTEAMS_BOT_SERVICE_BASE_URL': JSON.stringify(
          process.env.VITE_MSTEAMS_BOT_SERVICE_BASE_URL
        ),
        'process.env.VITE_MSTEAMS_CLIENT_API_KEY': JSON.stringify(
          process.env.VITE_MSTEAMS_CLIENT_API_KEY
        ),
      },
    };
